...
class StorageDevice(Device):
    ...
    def __init__(self, name, fmt=None, uuid=None,
        ...
        # XXX Should we trust passed in size or always consult /sys?
        #     LVM snapshots probably need us to trust passed-in value.
    ...
    def _getSize(self):
        ...
        size = self._size
        if self.exists and self.resizable and self.targetSize:
    ...
    def readCurrentSize(self):
        log_method_call(self, exists=self.exists, path=self.path,
                        sysfsPath=self.sysfsPath, node_exists=os.path.exists(self.path))
        size = Size(0)
