...
class LUKSFormatPopulator(FormatPopulator):
    ...
    def run(self):
        ...
        if not self._devicetree.get_device_by_name(self.device.format.map_name):
            passphrase = luks_data.devs.get(self.device.format.uuid)
            if self.device.format.configured:
                pass
            elif passphrase:
                self.device.format.passphrase = passphrase
            elif self.device.format.uuid in luks_data.devs:
                ...
                # Try each known passphrase. Include luks_data.devs values in case a
                ...
                passphrases = luks_data.passphrases + list(luks_data.devs.values())
