...
class Ext2FSMinSize(FSMinSize):
    ...
    def do_task(self):  # pylint: disable=arguments-differ
        ...
        if block_size is None:
            raise FSError("failed to get block size for {} filesystem on {}".format(self.fs.mount_type, self.fs.device))

        resize_info = self._get_resize_info()
        num_blocks = self._extract_num_blocks(resize_info)
        if num_blocks is None:
            raise FSError("failed to get minimum block number for {} filesystem on {}".format(self.fs.mount_type, self.fs.device))
