...
class Blivet(object, metaclass=SynchronizedMeta):
    ...
    def new_mdarray(self, *args, **kwargs):
        ...
        if name:
            safe_name = self.safe_device_name(name, devicefactory.DEVICE_TYPES.MD)
            if safe_name != name:
    ...
    def new_vg(self, *args, **kwargs):
        ...
        name = kwargs.pop("name", None)
        if name:
            safe_name = self.safe_device_name(name, devicefactory.DEVICE_TYPES.LVM)
            if safe_name != name:
                log.warning("using '%s' instead of specified name '%s'",
                            safe_name, name)
                name = safe_name
        else:
            name = self.suggest_container_name(container_type=devicefactory.DEVICE_TYPES.LVM)
    ...
    def new_lv(self, *args, **kwargs):
        ...
        if name:
            ...
            safe_vg_name = self.safe_device_name(vg.name, devicefactory.DEVICE_TYPES.LVM)
            full_name = "%s-%s" % (safe_vg_name, name)
            safe_name = self.safe_device_name(full_name, devicefactory.DEVICE_TYPES.LVM)
    ...
    def new_btrfs(self, *args, **kwargs):
        ...
        if kwargs.pop("subvol", False):
            ...
            if not name:
                name = self.suggest_container_name(container_type=devicefactory.DEVICE_TYPES.BTRFS)
    ...
    def new_stratis_pool(self, *args, **kwargs):
        ...
        name = kwargs.pop("name", None)
        if name:
            safe_name = self.safe_device_name(name, devicefactory.DEVICE_TYPES.STRATIS)
            if safe_name != name:
                log.warning("using '%s' instead of specified name '%s'",
                            safe_name, name)
                name = safe_name
        else:
            name = self.suggest_container_name(container_type=devicefactory.DEVICE_TYPES.STRATIS)
    ...
    def new_stratis_filesystem(self, *args, **kwargs):
        ...
        if name:
            ...
            safe_name = self.safe_device_name(full_name, devicefactory.DEVICE_TYPES.STRATIS)
            if safe_name != full_name:
            ...
            name = self.suggest_device_name(parent=pool,
                                            mountpoint=mountpoint,
                                            device_type=devicefactory.DEVICE_TYPES.STRATIS)
    ...
    def safe_device_name(self, name, device_type=None):
        ...
        if device_type in (devicefactory.DEVICE_TYPES.LVM, devicefactory.DEVICE_TYPES.LVM_THINP):
            allowed = devicelibs.lvm.safe_name_characters
        elif device_type == devicefactory.DEVICE_TYPES.MD:
            allowed = devicelibs.mdraid.safe_name_characters
        elif device_type == devicefactory.DEVICE_TYPES.BTRFS:
            allowed = devicelibs.btrfs.safe_name_characters
        elif device_type == devicefactory.DEVICE_TYPES.STRATIS:
    ...
    def unique_device_name(self, name, parent=None, name_set=True, device_type=None):
        ...
        if device_type == devicefactory.DEVICE_TYPES.STRATIS:
            parent_separator = "/"
        else:
            parent_separator = "-"
    ...
    def factory_device(self, device_type=devicefactory.DEVICE_TYPES.LVM, **kwargs):
        ...
        if kwargs["fstype"] == "swap" and \
           device_type == devicefactory.DEVICE_TYPES.BTRFS:
            device_type = devicefactory.DEVICE_TYPES.PARTITION
