...
To maintain Python 3.9 support, avoid using the following (incomplete list, subject to change as python evolves):
- Structural pattern matching (match / case) (3.10+)
- Union types with | syntax (e.g. int | str) → use typing.Union
- typing.Self, typing.override, ParamSpec, TypeVarTuple (3.11+)
- Exception groups / except* (3.11+)
- asyncio.TaskGroup (3.11+) → use manual task handling
- tomllib standard library module (3.11+) → use third-party toml
- Enhanced f-string syntax (3.12+) (keep expressions simple)
- Subinterpreter / per-interpreter APIs (3.12+)
